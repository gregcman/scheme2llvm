...
(defun llvm-store? (exp)
  (tagged-list? exp 'store))
(defun llvm-getelementptr? (exp)
  (tagged-list? exp 'getelementptr))
(defun llvm-ptrtoint? (exp)
  (tagged-list? exp 'ptrtoint))
(defun llvm-inttoptr? (exp)
  (tagged-list? exp 'inttoptr))
(defun llvm-ret? (exp)
  (tagged-list? exp 'ret))
...
(defun lambda-parameters (exp) 
  (labels ((fix-list (lst)
	...
	(fix-list (cadr exp)))))
(defun lambda-arbitrary-args? (exp)
  (not (list? (cadr exp))))
...
(defun compiled-code (compile-res)
  (cdr compile-res))
(defun compiled-target (compile-res)
  (car compile-res))

(defun append-code2 (instrs)
  (cond ((null? instrs) instrs)
	...
	(else
	 (cons (car instrs)
	       (append-code2 (cdr instrs))))))
(defun append-code (&rest instrs)
  (append-code2 instrs)) 
(defun make-code (target &rest instrs)
  (cons target
	(append-code2 instrs)))
...
(defparameter function-counter 0)
(defun make-raw-function-name ()
...
(defparameter llvm-primitive-functions '())
(defun add-llvm-function-name (f-name)
  (set! llvm-primitive-functions
	(cons f-name llvm-primitive-functions)))

(defparameter llvm-function-list '())
(defun add-llvm-function (f-name f-params f-body)
...
(defun fix-string-format (str) ;; some extra work since we have no char-type.
  (labels ((str-ref-int (str pos)
	     (char->integer (string-ref str pos)))
	   (esc-char (hex1 hex2 rest)
	     (cons (integer->char 92) 
		   (cons (integer->char hex1)
			 (cons (integer->char hex2)
			       rest))))
	   (fix-str-format (str pos end)
	     (cond ((= pos end) '())
...
(defparameter llvm-string-list '())
(defun add-llvm-string (target str)  
...
(defun extend-c-t-env (params c-t-env)
  (cons params c-t-env))
(defun current-c-t-env (c-t-env)
  (car c-t-env))
(defun outer-c-t-env (c-t-env)
  (cdr c-t-env))

(defun find-var (var c-t-env scope)
  (labels ((find-index (var env index)
	     (cond ((null? env) '())
...
(defparameter llvm-instructions
  '(;; binary operations.
...
(defparameter llvm-boolean-instructions
  '(seteq setne setlt setgt setle setge))
(defparameter llvm-shift-instructions
  '(bit-shl bit-shr))
(defun llvm-instr-name (op)
  (cdr (assoc op llvm-instructions)))

(defun llvm-global-repr (exp)
  (cond ((number? exp)
	 (number->string exp))
	((symbol? exp)
	 (c "@\"fun-" (symbol->string exp) "\""))
        (else exp)))
...
(defun llvm-instruction (target op x y)
  (c target " = " (llvm-instr-name op) " i64 " 
     (llvm-repr x) ", " (llvm-repr y)))

;; Identity function
(defun llvm-id (target exp) 
  (llvm-instruction target 'add 0 exp))

(defun llvm-call2 (target function args)
  (labels ((build-arg-list (arg-list fi)
	     (if (null? arg-list) ""
		 (c (if (= fi 1) "" ", ")
...
(defun llvm-call (target function &rest args)
  (llvm-call2 target function args))

(defun llvm-ret (value)
  (c "ret i64 " (llvm-repr value)))

(defun llvm-ptrtoint (target type1 x type2)
  (c target " = ptrtoint " type1 " " x " to " type2))

(defun llvm-inttoptr (target type1 x type2)
  (c target " = inttoptr " type1 " " x " to " type2))

(defun llvm-phi (target type vars)	; not used
  (labels ((build-phi-list (phis)
	     (c "[" (llvm-repr (car (car phis))) ", %" (cdr (car phis)) "]"
		(if (null? (cdr phis)) "" 
		    (c ", " (build-phi-list (cdr phis)))))))
    (c target " = phi " type " " (build-phi-list vars)))) 

(defun llvm-label (label)
  (c label ":"))
(defun llvm-br (label)
  (c "br label %" label))
(defun llvm-bool-br (pred c-label a-label)
  (let ((t1 (make-var))
	(t2 (make-var)))
    (append-code
...
(defun llvm-store (target value) 
  (c "store i64 " value ", i64* " target))

(defun llvm-load (target var)
  (c target " = load i64, i64* " var))
(defun llvm-alloca-var (target)
  (c target " = alloca i64"))

(defun llvm-shift-op (target op value sh)
  (c target " = " (llvm-instr-name op) " i64 " (llvm-repr value) ", " sh))

(defun llvm-getelementptr (target ptr index)
  (let ((t1 (make-var)))
    (append-code
     (llvm-inttoptr t1 "i64" ptr "i64*")
     (c target " = getelementptr i64, i64* " t1 ", i64 " index))))

(defun llvm-vector-ref (target vector index)
  (llvm-call target 'vector-ref (llvm-repr vector) (llvm-repr index)))

(defun llvm-vector-set! (vector index value)
  (llvm-call (make-var) 'vector-set! (llvm-repr vector)
	     (llvm-repr index) (llvm-repr value)))

(defun block-comment (str exp)
  (display (c ";>>> " str))
  (newline)
  (display exp)
  (newline)
...
(defun compile-self-evaluating (exp c-t-env)
...
(defun compile-variable (exp c-t-env)
...
(defun compile-assignment (exp c-t-env)
  (let ((target (make-var))
...
(defun compile-sequence (seq c-t-env)
  (labels ((sequence-defuns (seq)
	     (cond ((null? seq) '())
		   ((definition? (car seq))
		    (cons (definition-variable (car seq)) (sequence-defuns (cdr seq))))
		   ((llvm-definition? (car seq))
		    (add-llvm-function-name (definition-variable (car seq)))
		    (sequence-defuns (cdr seq)))
		   (else (sequence-defuns (cdr seq)))))
	   
	   (append-sequences (seq code-seq c-t-env2)
	     (if (last-exp? seq)
    ...
    (let ((seq-defuns (sequence-defuns seq)))
      (if (null? seq-defuns) ;; no local definitions
	  (append-sequences seq '() c-t-env)
	  (let ((seq-code 
		 (append-sequences seq '() (extend-c-t-env seq-defuns c-t-env)))
		(f-name (make-function-name))
		(t1 (make-var)) 
		(seq-env (make-var))
		(target (make-var)))
	    (declare (ignore seq-env)) ;;;;FIXME:: What is seq-env for?
	    (add-llvm-function f-name '(env) seq-code)
	    (make-code 
	     target
	     (llvm-call t1 'make-env (%length seq-defuns) 'env)
	     (llvm-call target f-name t1)))))))

(defun compile-lambda-with-name (exp c-t-env f-name)
  (let ((t1 (make-var))
	(target (make-var))
	(l-body 
...
(defun compile-lambda (exp c-t-env)
  (compile-lambda-with-name exp c-t-env (make-function-name)))

(defun compile-named-lambda (exp c-t-env def-name)
  (compile-lambda-with-name exp c-t-env (c "@\"" (make-raw-function-name) "-" (symbol->string def-name) "\"")))

(defun compile-llvm-definition (exp c-t-env)
  (declare (ignore c-t-env)) ;;FIXME: What is c-t-env for?
  (let ((f-name (definition-variable exp))
	(f-lambda (definition-value exp))
	(target (make-var))) ;;FIXME: What is target for?
...
(defun compile-llvm-instruction (exp c-t-env)
  (let ((target (make-var)))
    (cond 
      ((llvm-load? exp)
...
(defun compile-application (exp c-t-env)
  (labels ((build-param-list (param-list operand-codes index)
	...
	(let ((func (make-var))
	      ...
	      (operand-codes (mapcar (lambda (operand) (%compile operand c-t-env))
				     (operands exp))))
	  (make-code 
	   target
...
(defun compile-llvm-application (exp c-t-env)
  (let ((target (make-var))
	(operand-codes (mapcar (lambda (operand) (%compile operand c-t-env))
			       (operands exp))))
    (make-code target 
	       (append-code2 (mapcar #'compiled-code operand-codes))
	       (llvm-call2 target (operator exp) 
			   (mapcar #'compiled-target operand-codes)))))

(defparameter bootstrap-llvm-code
  "
...
(defparameter bootstrap
  '(begin 
    ...
    (define (mapcar fn lst)
     (cond ((null? lst) '())
	   (else (cons (fn (car lst)) (mapcar fn (cdr lst))))))
...
(defun compiler (exp)
  ...
  (let ((printer (lambda (line) (display line) (newline)))
    ...
    (mapcar printer llvm-string-list)
    (display bootstrap-llvm-code)
    (display "define fastcc i64 @startup(i64 %\"env\") {\n")
    (mapcar printer (compiled-code result))
    (display (c "ret i64 " (compiled-target result) "\n}\n"))
    (display "; FUNCTIONS\n")
    (mapcar (lambda (function) (mapcar printer function) (newline))
	    llvm-function-list))
  0)
