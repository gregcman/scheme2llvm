...
(defclass module ()
  ((module :accessor module.module
   ...
   (live :accessor module.live :initarg :live)))
      ...
      (unwind-protect
	   (progn
	     (with-slots (module) module-object
	       (let ((module-abnormal? (k-shared::verify-llvm-module module)))
...
(cffi:define-foreign-library (libgc)
  (t (:default "libgc")))

(cffi:use-foreign-library libgc)

;;;;lf == (codegen ast)
(defun %bar (module-object &optional (name "main")) ;;;Whats lf?
  "Evalutate main"
  (declare (optimize (speed 0) (debug 3)))
  (with-slots (module live) module-object   
